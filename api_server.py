...
#  Pings /api/market-regime every 10 minutes to keep dyno warm
# ═══════════════════════════════════════════════════════════════════════════

def keep_alive():
    """Background thread that pings the server every 10 minutes to prevent sleep."""
    import requests
    import time
    ...
    # Self-ping URL — change if deployed elsewhere
    ...
    logger.info(f"Keep-alive thread started — will ping {ping_url} every 10 minutes")
...
# Start keep-alive thread as daemon (dies when main app dies)
threading.Thread(target=keep_alive, daemon=True).start()
logger.info("Keep-alive thread launched — Render dyno will stay warm")
